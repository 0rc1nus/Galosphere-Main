...
public class PinkSaltStrawPatchFeature extends Feature<PinkSaltStrawPatchConfig> {
                    ...
                    for (int y = -yRange; y <= yRange; y++) {
                        ...
                        if (x * x + z * z <= xRadius * zRadius) {
                            if (world.getBlockState(pos).is(GBlockTags.PINK_SALT_BLOCKS) && world.isStateAtPosition(pos.relative(direction), DripstoneUtils::isEmptyOrWater)) {
                                if (x == 0 && z == 0) {
                                    world.setBlock(pos.relative(direction), GBlocks.PINK_SALT_CLUSTER.get().defaultBlockState().setValue(PinkSaltLampBlock.WATERLOGGED, world.getBlockState(pos.relative(direction)).is(Blocks.WATER)).setValue(PinkSaltLampBlock.FACING, direction), 2);
                                } else {
    ...
    public void addSaltStraw(WorldGenLevel world, BlockPos blockPos, int height, Direction direction) {
        for (int i = 0; i <= height; i++) {
            ...
            world.setBlock(pos, GBlocks.PINK_SALT_STRAW.get().defaultBlockState().setValue(PinkSaltStrawBlock.TIP_DIRECTION, direction).setValue(PinkSaltStrawBlock.STRAW_SHAPE, strawShape).setValue(PinkSaltStrawBlock.WATERLOGGED, world.getBlockState(pos).is(Blocks.WATER)), 2);
        }
    }
