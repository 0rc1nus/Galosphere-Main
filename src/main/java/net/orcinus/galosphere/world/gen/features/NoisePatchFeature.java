                ...
                if (random.nextInt(30) == 0) {
                    for (Direction direction : Direction.values()) {
                        ...
                        if (random.nextInt(10) == 0 && world.isStateAtPosition(clusterPos, DripstoneUtils::isEmptyOrWater)) {
                            world.setBlock(clusterPos, GBlocks.PINK_SALT_CLUSTER.get().defaultBlockState().setValue(PinkSaltLampBlock.FACING, direction).setValue(PinkSaltLampBlock.WATERLOGGED, world.getBlockState(clusterPos).is(Blocks.WATER)), 2);
                        }
                    }
                }
