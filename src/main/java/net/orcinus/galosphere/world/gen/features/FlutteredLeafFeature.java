    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (world.isStateAtPosition(blockPos, DripstoneUtils::isEmptyOrWater) && world.getBlockState(blockPos.below()).isFaceSturdy(world, blockPos.below(), Direction.UP)) {
            ...
            for (int i = 0; i <= height; i++) {
                FlutterFrondBlock.FrondShape shape = i > 0 ? i == height ? FlutterFrondBlock.FrondShape.TOP : FlutterFrondBlock.FrondShape.MIDDLE : FlutterFrondBlock.FrondShape.BOTTOM;
                BlockPos placePos = blockPos.above(i);
                if (world.isStateAtPosition(placePos, DripstoneUtils::isEmptyOrWater)) {
                    world.setBlock(placePos, CTBlocks.FLUTTER_FROND.get().defaultBlockState().setValue(FlutterFrondBlock.SHAPE, shape).setValue(FlutterFrondBlock.WATERLOGGED, world.getFluidState(placePos).getType() == Fluids.WATER), 2);
                } else {
                    world.setBlock(placePos, CTBlocks.FLUTTER_FROND.get().defaultBlockState().setValue(FlutterFrondBlock.SHAPE, shape).setValue(FlutterFrondBlock.WATERLOGGED, world.getFluidState(placePos).getType() == Fluids.WATER), 2);
                    break;
                }
