...
public class LaySpecterpillar extends Behavior<SpectreEntity> {
    ...
    public LaySpecterpillar() {
        super(ImmutableMap.of(GMemoryModuleTypes.NEAREST_LICHEN_MOSS, MemoryStatus.VALUE_PRESENT, MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_PRESENT, MemoryModuleType.IS_PREGNANT, MemoryStatus.VALUE_PRESENT));
    }

    @Override
    protected boolean checkExtraStartConditions(ServerLevel serverLevel, SpectreEntity livingEntity) {
        return !livingEntity.getBlockStateOn().is(GBlocks.LICHEN_MOSS);
    }
    ...
    protected void start(ServerLevel serverLevel, SpectreEntity livingEntity, long l) {
        this.getNearstLichenMoss(livingEntity).filter(blockPos -> serverLevel.getBlockState(blockPos.below()).is(GBlocks.LICHEN_MOSS)).ifPresent(blockPos -> {
    ...
    protected void stop(ServerLevel serverLevel, SpectreEntity livingEntity, long l) {
        if (!livingEntity.getBlockStateOn().is(GBlocks.LICHEN_MOSS)) {
            ...
            SpecterpillarEntity specterpillar = GEntityTypes.SPECTERPILLAR.create(serverLevel);
    ...
    private Optional<BlockPos> getNearstLichenMoss(SpectreEntity livingEntity) {
        return livingEntity.getBrain().getMemory(GMemoryModuleTypes.NEAREST_LICHEN_MOSS);
    }
