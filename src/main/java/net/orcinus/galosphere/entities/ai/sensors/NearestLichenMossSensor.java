    ...
    protected void doTick(ServerLevel world, Specterpillar entity) {
        ...
        for (int x = -range; x <= range; x++) {
            for (int z = -range; z <= range; z++) {
                for (int y = -range; y <= range; y++) {
                    ...
                    if (state.isAir() && belowState.is(GBlocks.LICHEN_MOSS)) {
                        poses.add(pos);
                    }
                }
            }
        ...
        if (!poses.isEmpty()) {
            ...
            for (BlockPos pos : poses) {
                ...
                if (!entity.getBrain().hasMemoryValue(GMemoryModuleTypes.NEAREST_LICHEN_MOSS) && path != null && path.canReach()) {
                    entity.getBrain().setMemory(GMemoryModuleTypes.NEAREST_LICHEN_MOSS, pos);
                }
            }
        } else {
            entity.getBrain().eraseMemory(GMemoryModuleTypes.NEAREST_LICHEN_MOSS);
        }
    }
