...
public class SendParticlesPacket implements ClientPlayNetworking.PlayChannelHandler {
    ...
    public void receive(Minecraft client, ClientPacketListener handler, FriendlyByteBuf buf, PacketSender responseSender) {
        ...
        client.execute(() -> {
            ...
            world.playLocalSound(blockPos, SoundEvents.SCULK_BLOCK_CHARGE, SoundSource.BLOCKS, 1.0F, 1.0F, false);
            boolean flag = world.getBlockState(blockPos).isCollisionShapeFullBlock(world, blockPos);
            int l2 = flag ? 40 : 20;
            float f9 = flag ? 0.45F : 0.25F;
            for (int k3 = 0; k3 < l2; ++k3) {
                float f12 = 2.0F * random.nextFloat() - 1.0F;
                float f14 = 2.0F * random.nextFloat() - 1.0F;
                float f15 = 2.0F * random.nextFloat() - 1.0F;
